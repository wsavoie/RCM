...
bool Smarticle::checkNotColliding(Smarticle other, Smarticle main)
{
	...
	bool Smarticle::checkEndPts()
	{
		...
		for (unsigned i = 0; i < 2; i++)
		{
			...
			if (sqrt(x*x + y*y + z*z) > boxSize / 2.0)  //if position's r is greater than sphere radius
				return false;
	...
	//#include <math.h>
	//namespace gte {
gte::Vector<3, double> smart::genVec(double x, double y, double z)
{
...
gte::Segment3<double> smart::genSeg(double x1, double y1, double z1, double x2, double y2, double z2)
{
	...
	p0 = smart::genVec(x1, y1, z1);
	resultSeg.p.at(0) = p0;
	
	p1 = smart::genVec(x2, y2, z2);
	resultSeg.p.at(1) = p1;
...
gte::Vector<3, double> smart::crossProd(gte::Vector<3, double> u, Vector<3, double> v)
{
...
gte::Vector<3, double> smart::rotateByAngle(double ang, gte::Vector<3, double> point0, gte::Vector<3, double> point1, gte::Vector<3, double> b)
{//https://en.wikipedia.org/wiki/Rodrigues%27_rotation_formula
	//v == a from wiki
	gte::Vector<3, double> result, v, k;
	v = (point1 - point0);
	k = smart::crossProd(v, b);
	k = k / gte::Length(k);
	//result = v*cos(ang) + crossProd(k, v)*sin(ang) + k*(Dot(k, v))*(1 - cos(ang)) + point0;
	
	result = v*cos(ang) + smart::crossProd(k, v)*sin(ang) + k*(Dot(k, v))*(1 - cos(ang)) + point0;
	return result;
}
