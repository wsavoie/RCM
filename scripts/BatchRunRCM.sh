...
for z in `seq 0 14`; do
	...
	a1Arr=(90);
	a2Arr=(90);
	boxSizeArr=(350 350 350 350 350 350 350 350 350 350 350 350 350 350 350);
	angsL=${#a1Arr[@]};
	((angsLen=angsL-1));
	...
	for i in `seq 0 ${angsLen}`; do
	  ...
	  a=./$foldName/${lwArr[$z]}-${a1Arr[$i]}-${a2Arr[$i]}-${boxSizeArr[$z]}
	  mkdir $a
	  cd $a
	  $exePath ${a1Arr[$i]} ${a2Arr[$i]} ${lwVals[$z]} ${boxSizeArr[$z]};
	  printf '\nSim Time=%ds\n' "$SECONDS"
	  cd ../..;
	done;
done;